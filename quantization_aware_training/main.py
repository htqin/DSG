...
class Generator(nn.Module):
    def __init__(self, options=None, conf_path=None):
        ...
        self.conv_blocks2 = nn.Sequential(
            nn.Conv2d(128, 64, 3, stride=1, padding=1), nn.BatchNorm2d(
                64, 0.8), nn.LeakyReLU(0.2, inplace=False),
...
class Generator_imagenet(nn.Module):
    def __init__(self, options=None, conf_path=None):
        ...
        self.conv_blocks1_2 = nn.LeakyReLU(0.2, inplace=False)
        ...
        self.conv_blocks2_2 = nn.LeakyReLU(0.2, inplace=False)
...
class ExperimentDesign:
    ...
    def prepare(self):
        ...
        self.logger.info("self._set_dataloader() success!")
        self._set_model()
        self.logger.info("self._set_model() success!")
        self._replace()
        self.logger.info("self._replace() success!")
        self.logger.info(self.model)
        self._set_trainer()
        self.logger.info("self._set_trainer() success!")
    ...
    def _set_gpu(self):
        ...
        ), "Invalid GPU ID: " + str(self.settings.GPU) + "<=" + str(
            torch.cuda.device_count())
        print("Invalid GPU ID: " + str(self.settings.GPU) + "<=" +
    ...
    def _set_model(self):
        if self.settings.dataset in ["cifar100"]:
            ...
            print(self.arc)
            if self.arc.startswith('inception'):
                self.test_input = Variable(torch.randn(1, 3, 299, 299).cuda())
            else:
                self.test_input = Variable(torch.randn(1, 3, 224, 224).cuda())
            self.model = ptcv_get_model(self.arc, pretrained=True)
            self.model_teacher = ptcv_get_model(self.arc, pretrained=True)
    ...
    def _set_trainer(self):
        # set lr master
        lr_master_S = utils.LRPolicy(self.settings.lr_S, self.settings.nEpochs,
                                     self.settings.lrPolicy_S)
        lr_master_G = utils.LRPolicy(self.settings.lr_G, self.settings.nEpochs,
    ...
    def run(self, sci_coeff=1):
        best_top1 = 100
        best_top5 = 100
        start_time = time.time()

        from utils.datasets import ImageNetLoaderGenerator
        g = ImageNetLoaderGenerator(self.settings.dataPath,'imagenet',train_batch_size=32,test_batch_size=32,num_workers=0)
...
def main():
    ...
    experiment = ExperimentDesign(generator,
                                  option,
                                  arc=args.model)
    experiment.run()
