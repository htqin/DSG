...
class QuantAct(Module):
    ...
    def __init__(self,
        ...
        self.percentile = perc
        self.q_method = act_q_method
    ...
    def clip_val_minmax(self, x):
        x_min = x.data.min()
        x_max = x.data.max()
        self.x_min += -self.x_min + min(self.x_min, x_min)
        self.x_max += -self.x_max + max(self.x_max, x_max)

    def clip_val_ema(self, x):
        x_min, _ = x.data.view(x.data.size(0), -1).min(dim=1)
        x_max, _ = x.data.view(x.data.size(0), -1).max(dim=1)
        x_min = x_min.mean()
        x_max = x_max.mean()
        self.x_min += -self.x_min + min(self.x_min, x_min)
        self.x_max += -self.x_max + max(self.x_max, x_max)
    
    def clip_val_perc(self, x):
        x_np = x.cpu().numpy()
        x_min, x_max = np.percentile(x_np, (100 - self.percentile, self.percentile))
        #x_min = torch.from_numpy(np.asarray(x_min)).cuda()
        #x_max = torch.from_numpy(np.asarray(x_max)).cuda()
        x_min = x_min * torch.ones(1).cuda()
        x_max = x_max * torch.ones(1).cuda()
        self.x_min += -self.x_min + min(self.x_min, x_min)
        self.x_max += -self.x_max + max(self.x_max, x_max)

    def clip_val_mse(self, x):
        best_score = 1e6
        x_max = x.data.max()
        x_min = x.data.min()
        ...
        for i in range(0, 80):
            gap = x_max - x_min
            new_max = (x_max - i * gap * 0.01) * torch.ones(1).cuda()
            #new_min = (x_min + i * gap * 0.01) * torch.ones(1).cuda()
            new_min = x_min
            x_1 = x.flatten()
            quant_x = self.act_function(x_1, self.activation_bit, new_min, new_max)
            score = torch.norm(x_1 - quant_x)
            if score <= best_score:
                best_score = score
                best_i = i
                best_min = new_min * torch.ones(1).cuda()
                best_max = new_max * torch.ones(1).cuda()
        # print(best_i, best_score)
        self.x_min = best_min
        self.x_max = best_max
        #self.x_min += -self.x_min + min(self.x_min, best_min)
        #self.x_max += -self.x_max + max(self.x_max, best_max)
    ...
    def forward(self, x):
        ...
        if self.running_stat:
            ...
            #self.x_min += -self.x_min + min(self.x_min, x_min)
            #self.x_max += -self.x_max + max(self.x_max, x_max)
            if self.q_method == 'minmax':
                self.clip_val_minmax(x)
            elif self.q_method == 'ema':
                self.clip_val_ema(x)
            elif self.q_method == 'perc':
                self.clip_val_perc(x)
