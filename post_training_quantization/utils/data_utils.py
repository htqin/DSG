...
def generate_sample(size):
    ...
    x = x.view(size[0], -1) 
    dim = x.shape[1] 
    points, weights = subsampled_dense_grid(dim, dim / 2, gamma=0.01)
    x_transformed = rbf_feature_map(x, points, weights)
    x_transformed = torch.tensor(x_transformed).view(size).float()
...
class GaussDataset(Dataset):
    ...
    def __getitem__(self, idx):
        sample = torch.rand(size = self.size)
        return sample
...
def getRandomData(dataset='cifar10', batch_size=512, for_inception=False):
    ...
    dataset = GaussDataset(length=10000, size=size, transform=None) # gauss initialization
